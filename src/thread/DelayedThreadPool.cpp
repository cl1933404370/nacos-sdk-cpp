    ...
    void run() override
    {
        ...
        while (!_container._stop_delayed_tp) {
            ...
            * */
            ...
            while (!_container._scheduledTasks.empty()) {
                const auto it = _container._scheduledTasks.begin();
	            const int64_t now_time = TimeUtils::getCurrentTimeInMs();
                ...
                if (it->first <= now_time) {
                   ...
                   _container._lockForScheduleTasks.lock(); 
                    ...
                    _container._delayTaskNotEmpty.wait([](){return true;},it->first - now_time);
                }
            }
...
struct tagAscOrdFunctor{
    bool operator ()(const std::pair<int64_t, Task*> &lhs, const std::pair<int64_t, Task*> &rhs) const
    {
...
void DelayedThreadPool::schedule(Task *t, int64_t futureTimeToRun) {
    if (_stop) {
        return;
    }
    ...
    {
        const std::pair<int64_t, Task*> scheduledTask = std::make_pair(futureTimeToRun, t);
...
void DelayedThreadPool::stop() {
    ...
    _delayTaskNotEmpty.notifyAll();
    for (const Thread *  _thread : _threads)
    {
        _thread->kill();
    }
