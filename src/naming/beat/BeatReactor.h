...
class BeatReactor {
    ...
    std::atomic_uint64_t _clientBeatInterval;
protected:
    friend class BeatTask;
    volatile bool _stop;
    DelayedThreadPool *_delayedThreadPool;
public:
    void setClientBeatInterval(const uint64_t interval) { _clientBeatInterval = interval; }

    uint64_t getClientBeatInterval() const { return _clientBeatInterval; }

    BeatReactor(ObjectConfigData *objectConfigData, const int threadCount)
            : _objectConfigData(objectConfigData) {
        _threadCount = threadCount;
        _stop = true;
        _clientBeatInterval = static_cast<uint64_t>(5) * 1000;
